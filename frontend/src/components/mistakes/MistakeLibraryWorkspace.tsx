...
function StudyView({item,solutionRevealed,solutionRegionId,onReveal,onOpenLine}:{item:SavedMistake;solutionRevealed:boolean;solutionRegionId:string;onReveal:()=>void;onOpenLine:()=>void}){
  ...
  return <div className="mistake-study-view">
    ...
    <div className="mistake-study-ledger"><section className="mistake-solution-card" aria-label="Mistake solution"><div id={solutionRegionId} className="mistake-solution-ledger" role={solutionRevealed?'region':undefined} aria-label={solutionRevealed?'Best move and line':undefined}><div className="mistake-verdict"><span>Played<strong>{item.played_move}</strong></span><i aria-hidden="true">→</i><span>Best<strong className={solutionRevealed?undefined:'mistake-solution-hidden'}>{solutionRevealed?(item.best_move??'Best move unavailable'):'Hidden'}</strong></span></div><div className="mistake-solution-row"><div className="mistake-line"><span>Best Line</span><p className={solutionRevealed?undefined:'mistake-solution-placeholder'}>{solutionRevealed?(preview||'No stored line'):'Hidden until revealed'}</p>{solutionRevealed&&line.length>8&&<button type="button" className="text-button mistake-line-expand" onClick={onOpenLine}>View complete line</button>}</div><button type="button" className="primary-button mistake-solution-reveal" aria-expanded={solutionRevealed} aria-controls={solutionRegionId} onClick={onReveal} disabled={solutionRevealed}>{solutionRevealed?'Best Move Revealed':'Reveal Best Move'}</button></div></div></section>
